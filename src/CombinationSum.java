public class Solution {
    public ArrayList<ArrayList<Integer>> combinationSum(int[] candidates, int target) {
        ...
        comboSum(candidates, 0, target, 0, new ArrayList<Integer>(), res);
        return res;
    }

    private void comboSum(int[] candidates, int sum,  int target, int pos, ArrayList<Integer> pRes, ArrayList<ArrayList<Integer>> res){
        for(int i= pos; i< candidates.length; i++){
            ...
            if(sum == target)
                ...
                comboSum(candidates, sum, target, i, pRes, res);
