...
class Solution {
    public boolean areSentencesSimilarTwo(String[] words1, String[] words2, List<List<String>> pairs) {
        if (words1.length != words2.length)
            return false;
        Map<String, String> parents = new HashMap<>(); // mapping for UF

        for (List<String> p : pairs) {
            String w0 = p.get(0);
            String w1 = p.get(1);
            if (!parents.containsKey(w0))
                parents.put(w0, w0);
            if (!parents.containsKey(w1))
                parents.put(w1, w1);
            String p0 = find(parents, w0);
            String p1 = find(parents, w1);

            if (!p0.equals(p1))
                parents.put(p0, p1); // union
        }
        ...
        for (int i = 0; i < words1.length; i++) {
            ...
            if (!parents.containsKey(words1[i]) || !parents.containsKey(words2[i]))
                return false;
            if (!find(parents, words1[i]).equals(find(parents, words2[i])))
                return false;
        }
