...
// Choose k from n numbers. Make sure each number is selected with the probability of k/n
// (special case: k=1)
...
class Solution {
    ...
    public int getRandom() {
        ...
        for(int i = 1; cur.next != null; i++) {
            cur = cur.next;
            if(rand.nextInt(i + 1) == 0) { // use nextInt to get and
                res = cur.val;             // replace with 1 / (1 + i) odd
            }
        }
        return res;
    }
